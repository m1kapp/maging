...
(function () {
  ...
  .scroll-table .mw-table__wrap { flex: 1; min-height: 0; overflow: hidden; }
  .scroll-table .mw-table { width: 100%; height: 100%; font-size: var(--mw-text-xs); border-collapse: separate; border-spacing: 0; table-layout: fixed; }
  .scroll-table .mw-table th, .scroll-table .mw-table td { padding: var(--mw-space-3-5) var(--mw-space-2); font-variant-numeric: tabular-nums; background: var(--mw-surface); text-align: right; overflow: hidden; text-overflow: ellipsis; }
  .scroll-table .mw-table th:first-child, .scroll-table .mw-table td:first-child { text-align: left; }
...
`;
  ...
  function initDeck(MOUNT) {
    ...
    function activateById(id) {
      ...
      // Refresh only the active slide's charts (not all — hidden ones get 0 size)
      setTimeout(() => {
        if (typeof Maging !== 'undefined' && Maging.refreshAll) {
          target.querySelectorAll('.mw-chart__body').forEach(c => {
            var inst = typeof echarts !== 'undefined' && echarts.getInstanceByDom(c);
            if (inst) inst.resize();
          });
        }
      }, 50);
    ...
    function scaleSlide() {
      const SLIDE_W = 1240;
      const SLIDE_H = SLIDE_W * (210 / 297);

      if (document.body.classList.contains('presenter-mode')) {
        // Presenter: scale to fill viewport, centered
        const vw = window.innerWidth, vh = window.innerHeight;
        const scaleX = vw / SLIDE_W;
        const scaleY = vh / SLIDE_H;
        const scale = Math.min(scaleX, scaleY);
        slides.forEach(s => {
          s.style.width = '';
          s.style.height = '';
          s.style.aspectRatio = '';
          s.style.setProperty('--slide-scale', scale.toFixed(4));
        });
        // Center the slide in viewport
      ...
      // Normal mode: scale to fit container width
      const main = document.querySelector('main');
      if (main) { main.style.display = ''; main.style.alignItems = ''; main.style.justifyContent = ''; main.style.width = ''; main.style.height = ''; }

      // Use main content area width as reference
      const mainEl = document.querySelector('main');
      const containerW = mainEl ? mainEl.clientWidth - 48 : window.innerWidth; // 48 = px-6 * 2
      const scale = containerW < SLIDE_W ? containerW / SLIDE_W : 1;
      ...
      slides.forEach(s => {
        ...
        s.style.aspectRatio = '';
        s.style.setProperty('--slide-scale', scale.toFixed(4));
        s.style.marginBottom = '-' + (SLIDE_H * (1 - scale)).toFixed(0) + 'px';
      });
    }
