...
maging 위젯은 컨테이너 기반 반응형이다. 뷰포트가 아니라 **부모 요소 너비**에 맞춘다.
...
| (기본) | ~640px | 모바일 — 1열 스택 |
| `md:` | 768px+ | 태블릿 — 2열 |
| `lg:` | 1024px+ | 데스크톱 — 2~4열 비대칭 |

### 모바일 규칙

- 모든 `grid-cols-N`은 `grid-cols-1 md:grid-cols-N`으로 작성. 모바일에서 자동 1열 스택.
- KPI 카드: 모바일에서 `grid-cols-2` (4개 → 2×2). `grid-cols-1`로 세로 나열하지 마라.
- 차트: `ResizeObserver` 기반 자동 리사이즈. 별도 처리 불필요.
- 주간보고 슬라이드: 모바일에서 `transform: scale()`로 축소 + 좌우 스와이프.
- 카드뉴스: 4:5 비율 유지, `max-width: 540px` 자동 제한.
- 사이드바: 모바일에서 숨김 → 햄버거 메뉴.
- 테이블: `scroll-table` wrapper로 가로 스크롤 처리. 컬럼 숨기지 마라.
